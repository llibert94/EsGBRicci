    ...
    const Coordinates<data_t> &coords) const
{
    ...
    // add quadratic terms
    SGB += 4. * dfdphi * M * (dfdphi * RGB - dbetadphi * Zbeta) /
    ...
    FOR(i, j)
    {
        SijGB[i][j] = -2. / 3. * Omega_ij_TF[i][j] *
                      ...
                      2. * Omega / 3. * ( Fij[i][j] - vars.h[i][j] / 3. * (tr_covd2lapse * one_over_lapse - tr_A2) ) / chi_regularised +
    ...
    const Coordinates<data_t> &coords) const
{
    ...
    FOR(i1, j1)
    {
        ...
        FOR(i2, j2)
        {
            ...
            {
                if (j1 == j2)
                    LHS_mat[row][col] += 1. - 2. * G_factor * Omega / 3.;
                FOR(k)
                LHS_mat[row][col] +=
                    2. * G_factor * vars.chi * h_UU[j1][k] * Omega_ij_TF[j2][k];
            }
            if (j1 == j2)
                FOR(k)
            LHS_mat[row][col] +=
                2. * G_factor * vars.chi * h_UU[i1][k] * Omega_ij_TF[i2][k];
            if (i1 != j1)
            {
                LHS_mat[row][col] +=
                    -4. * G_factor *
                ...
                if (i1 == j2)
                {
                    if (i2 == j1)
                        LHS_mat[row][col] += 1. - 2. * G_factor * Omega / 3.;
                    FOR(k)
                    LHS_mat[row][col] += 2. * G_factor * vars.chi *
                                         h_UU[j1][k] * Omega_ij_TF[i2][k];
                }
                if (i2 == j1)
                    FOR(k)
                LHS_mat[row][col] +=
                    2. * G_factor * vars.chi * h_UU[i1][k] * Omega_ij_TF[j2][k];
            }
    ...
    FOR(i, j)
    {
        ...
        if (i != j)
            LHS_mat[idx][N - 2] +=
                G_factor * (Omega_ij_TF_UU_over_chi[j][i] -
    ...
    LHS_mat[N - 2][N - 1] = -2. * dbetadphi + 4. * dfdphi * M / 3.;
